#Loops through to check if the containers are loaded and ready for some randomization
...
$execute if score loaded temp matches 1.. in $(dimension) positioned $(x) $(y) $(z) run function spawner_tweaker:chests/randomizer/randomize_chest with storage spawner_tweaker:temp container_randomizer[$(n)]
...
#Looping
$execute if score n temp < $modification_limit spawner_tweaker run function spawner_tweaker:chests/randomizer/randomize_chest_loop with storage spawner_tweaker:temp container_randomizer[$(next)]
