...
function spawner_tweaker:run_function/attempt_execution with storage spawner_tweaker:temp Priming[0]
...
#Inciment function_timer and display
scoreboard players add function_timer st_priming 1
title @a actionbar [{"text":"Functions executed ","color":"gray"},{"score":{"name":"functions_executed","objective":"st_priming"},"color":"red"},{"text":"/"},{"score":{"name":"total_spawners","objective":"st_priming"},"color":"green"}]

#End when needed
execute if score functions_executed st_priming >= total_spawners st_priming run function spawner_tweaker:run_function/end
...
execute if score function_timer st_priming >= max temp run function spawner_tweaker:run_function/end